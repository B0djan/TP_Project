...
int ListeningNetworkSocket::make_listening_socket(char* const ip, uint16_t port) {
    struct sockaddr_in addr;
    int rc = inet_aton(ip, &addr.sin_addr);
    ASSERT(rc == 1, "failed to parse IP address");
    addr.sin_port = htons(port);   //  convert machine byte order to network order

    int fd = socket(AF_INET, SOCK_STREAM, 0);
    ASSERT(fd >= 0, "failed to create listening socket");

    int opt_value = 1;
    rc = setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &opt_value, sizeof(opt_value));  //  SO_REUSEADDR disables the error-reuse from bind
    ASSERT(!rc, "failed to set SO_REUSEADDR option for listening socket");

    int flags = fcntl(fd, F_GETFL);             //  transfer to non-blocking mode
    flags |= O_NONBLOCK;
