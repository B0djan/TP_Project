...
void SelectBasedEventLoop::on_writeable(int fd, ResCb cb) {
    const auto& [it, inserted] = on_writeable_callbacks.emplace(fd, cb);    //  save the associative array fd - сb
    ASSERT(inserted, "trying to track same fd twice");
}
...
void SelectBasedEventLoop::run_until_complete() {
    while (!on_readable_callbacks.empty() || !on_writable_callbacks.empty()) {  //  while there is at least one expected fd
        ...
        for (auto& [fd, cb] : on_writable_callbacks) {                          //  fill them with tracked fd in the class
            ...
            FD_SET(fd, &on_writable_callbacks);
            FD_SET(fd, &track_errors);
        }

        int active_fd;
        while ((active_fd = select(max_fd + 1, &track_readability, &on_writable_callbacks, &track_errors, NULL)) == -1) {  //  find the fd on which there was activity
